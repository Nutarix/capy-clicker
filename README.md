...
Казуальный портретный idle-кликер про капибар: цветы ускоряют прогресс семьи,
на 100% появляется новая капибара, одинаковых можно слить в более крупную.
Камера отдаляется по мере роста семьи. Есть грязевая лужа (временный ×2 к авто-прогрессу)
...
- Авто-прогресс семьи (~2%/с) без тапов + видимый shimmer / «+X%/с»
...
- HUD: «семья N/12» отдельно от «поляна: …»; floating «+N%» на тапы
...
- **Декор луга**: кусты/камень unlock на семье 3 / 6 / 9
...
- Сохранение семьи и прогресса через `shared_preferences`
